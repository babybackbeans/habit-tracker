...
function calendarIconSVG() {
  ...
  let html = "<svg viewBox='0 0 " + (gridWidth + 2) + " 28' width='40' height='40' xmlns='http://www.w3.org/2000/svg'>";
  html += "<text x='" + ((gridWidth + 2) / 2) + "' y='7' text-anchor='middle' font-family='DM Sans, sans-serif' font-weight='bold' font-size='6' fill='var(--color-text)'>" + monthLabel + "</text>";

  for (let i = 0; i < cells.length; i++) {
    if (cells[i]) {
      let col = i % cols;
      let row = Math.floor(i / cols);
      let x = 1 + col * (cellSize + gap);
